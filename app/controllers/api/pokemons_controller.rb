class Api::PokemonsController < ApplicationController
  ...
  def show
    ...
    moves = @pokemon.moves.map{ |m| {name: m.move.name, details: m.version_group_details.select{ |detail| detail.version_group.name == 'lets-go-pikachu-lets-go-eevee' } } }
                    .select{ |m| m[:details].present? }.flat_map{ |m| m[:details].map{ |detail| {name: m[:name], detail: detail } } }
                    .map do |m|
              {name: I18n.t('move.' + m[:name]), level_learned_at: m[:detail].level_learned_at,
               move_learn_method: m[:detail].move_learn_method.name  }
            end.sort_by{ |m| m[:level_learned_at] }
    ...
    render json: {
      ...
      flavor_text: @pokemon_species.flavor_text_entries.select do |entry|
                     entry.language.name == 'ja' && entry.version.name == 'lets-go-pikachu'
                   end.first&.flavor_text || @pokemon_species.flavor_text_entries.select do |entry|
